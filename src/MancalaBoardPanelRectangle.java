 ...
 *
 */
public class MancalaBoardPanelRectangle extends JPanel implements MouseListener, MouseMotionListener, ActionListener, MancalaBoardFormatter {

	private MancalaModel model;
	private Rectangle2D board;
	private MancalaPitGraphicsRectangle[] pits;
	...
	public MancalaBoardPanelRectangle(int width, int height, MancalaModel model) {
		...
		int pad = height / 20;
		stoneSize = 30;
		pits = new MancalaPitGraphicsRectangle[14];
		gameStarted = false;
		pitNoHighlight = -1;
		for (int i = 0; i < 14; i++)
			pits[i] = new MancalaPitGraphicsRectangle();
		...
		pits[6].setOuterBound(new Rectangle2D.Double(board.getX() + (pitWidth + 2 * pitPad) * 7 + pitPad,
				board.getY() + pitPad, pitWidth, bigPitHeight));
		pits[13].setOuterBound(new Rectangle2D.Double(board.getX() + pitPad, board.getY() + pitPad, pitWidth,
				bigPitHeight));
		...
		pits[6].setInnerBound(new Rectangle2D.Double(pits[6].getOuterBound().getX() + pitBoundPad,
				pits[6].getOuterBound().getY() + pitBoundPad, pitBoundWidth, bigPitBoundHeight));
		pits[13].setInnerBound(new Rectangle2D.Double(pits[13].getOuterBound().getX() + pitBoundPad,
				pits[13].getOuterBound().getY() + pitBoundPad, pitBoundWidth, bigPitBoundHeight));

		// Draw small pits
		double smallPitHeight = board.getHeight() / 2 - 2 * pitPad;
		for (int i = 0; i < 6; i++)
			pits[i].setOuterBound(new Rectangle2D.Double(board.getX() + (pitWidth + 2 * pitPad) * (i + 1) + pitPad,
					board.getY() + smallPitHeight + pitPad * 3, pitWidth, smallPitHeight));
		for (int i = 7; i < 13; i++)
			pits[i].setOuterBound(
					new Rectangle2D.Double(board.getX() + (pitWidth + 2 * pitPad) * (13 - i) + pitPad,
		...
		for (int i = 0; i < 6; i++)
			pits[i].setInnerBound(new Rectangle2D.Double(pits[i].getOuterBound().getX() + pitBoundPad,
					pits[i].getOuterBound().getY() + pitBoundPad, pitBoundWidth, smallPitBoundHeight));
		for (int i = 7; i < 13; i++)
			pits[i].setInnerBound(new Rectangle2D.Double(pits[i].getOuterBound().getX() + pitBoundPad,
					pits[i].getOuterBound().getY() + pitBoundPad, pitBoundWidth, smallPitBoundHeight));
...
//		board.setRoundRect(pad, pad, width - pad * 2, height - pad * 2, pitWidth + pad, pitWidth + pad);
	...
	public void randomizePosition(MancalaStoneGraphics stone, RectangularShape shape, boolean animate) {
		Rectangle2D shape1 = (Rectangle2D)shape;
		Random rand = new Random();
		double randX = shape1.getMinX() + (shape1.getMaxX() - shape1.getMinX()) * rand.nextDouble();
		double randY = shape1.getMinY() + (shape1.getMaxY() - shape1.getMinY()) * rand.nextDouble();

		// If the stone is not inside the shape, randomize again
		while (!shape1.contains(randX, randY)) {
			randX = shape1.getMinX() + (shape1.getMaxX() - shape1.getMinX()) * rand.nextDouble();
			randY = shape1.getMinY() + (shape1.getMaxY() - shape1.getMinY()) * rand.nextDouble();
		}
	...
	public void clearStones() {
		for (MancalaPitGraphicsRectangle holeGraphics : pits)
