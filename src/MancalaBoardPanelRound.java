 ...
 *
 */
public class MancalaBoardPanelRound extends JPanel implements MouseListener, MouseMotionListener, ActionListener, MancalaBoardFormatter {

	private MancalaModel model;
	private RoundRectangle2D board;
	private MancalaPitGraphicsRound[] pits;
	...
	public MancalaBoardPanelRound(int width, int height, MancalaModel model) {
		...
		int pad = height / 20;
		stoneSize = 30;
		pits = new MancalaPitGraphicsRound[14];
		gameStarted = false;
		pitNoHighlight = -1;
		for (int i = 0; i < 14; i++)
			pits[i] = new MancalaPitGraphicsRound();
	...
	public void randomizePosition(MancalaStoneGraphics stone, RectangularShape shape, boolean animate) {
		RoundRectangle2D shape1 = (RoundRectangle2D)shape;
	...
	public void clearStones() {
		for (MancalaPitGraphicsRound holeGraphics : pits)
